...
export default class ViewBookingDetailScreen extends Component<Props> {
  constructor (props) {
    super (props);
  ...
  getDataByID = async () => {
    ...
    let bookingNow = this.state.bookingID;

    await this.db.transaction (async tx => {
      const sql = `SELECT * FROM bookings WHERE bookID='${bookingNow}'`;
      tx.executeSql (sql, [], (tx, results) => {
        const len = results.rows.length;

        if (len) {
          this.setState ({booking: results.rows.item (0)});
        }
  ...
  deleteBooking () {
    Alert.alert ('Are You Sure?', 'Delete Your Booking', [
      {
        text: 'No',
        onPress: () => {},
      },
      {
        ...
        onPress: () => {
          this.db.transaction (tx => {
            tx.executeSql ('DELETE FROM bookings WHERE bookID=?', [
        ...
        <View style={styles.BookingWrapper}>
          <View style={styles.BookingContainer}>
            ...
            <View style={styles.bookingTopWrapper}>
              <Text style={styles.bookingItemBookingID}>
                Booking ID: {book ? book.bookID : ''} {/*emptySpace*/}
              </Text>
              {/* Edit Booking Access */}
              <TouchableOpacity
                onPress={() => {
                  this.props.navigation.navigate ('UpdateBookingScreen', {
                    id: book ? book.bookID : '',
                    refresh: this.getDataByID,
            ...
            <View style={styles.bookingMiddleWrapper}>
              <View style={styles.bookingDoctorItemsWrapper}>
                <View style={styles.bookingDoctorItemsImageWrapper}>
                  <Image
                    source={
                      book
                        ? docsData.getArray (docsData.doctors, book.docID).image
                        : ''
                    }
                    style={styles.bookingDoctorItemImage}
                  />
                </View>
                <View style={styles.bookingDoctorItemsTextWrapper}>
                  <Text style={styles.bookingDoctorItemName}>
                    {book
                      ? docsData.getArray (docsData.doctors, book.docID).name
                      : ''}
                  </Text>
                  <Text style={styles.bookingDoctorItemSpecialty}>
                    {book
                      ? docsData.getArray (docsData.doctors, book.docID)
                          .specialty
                      : ''}
                  </Text>
                </View>
            ...
            <View style={styles.bookingBottomWrapper}>
              ...
              <View style={styles.bookingRightItemsWrapper}>
                <Text style={styles.bookingRightItemText}>
                  {book ? book.date : ''}
                </Text>
                <Text style={styles.bookingRightItemText}>
                  {book ? book.time : ''}
